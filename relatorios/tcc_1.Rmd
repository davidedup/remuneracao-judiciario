...
# Os valores gastos com auxílios e benefícios são maiores que os de salários.
```{r echo = FALSE, message = FALSE, warning = FALSE, cache = FALSE, paged.print = FALSE}
# Stacked barchart
stacked_bar_chart <- dados_2016 %>%
  ...
  select(value, total_de__rendimentos, type, mes)

stacked_bar_chart %>% 
    ggplot(aes(fill=type, y=total_de__rendimentos, x=as.factor(mes))) +
